...
function listEmail({ ga, winnerName, entries, amzTag }) {
  ...
  const body = [
    ...
    `I pulled one entry at random and it was <b>${esc(winnerName)}</b>, so they're getting a hardcover of <b>${esc(ga.title)}</b> by ${esc(ga.author)}, which (perfect timing honestly) comes out today. Congrats to them, genuinely 💜`,
...
module.exports = { winnerEmail, listEmail, displayName, buyUrl, claimByDate, CLAIM_DAYS, PLAY_URL };
