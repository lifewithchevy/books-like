...
// Same signed one-click unsubscribe link api/booky-send.js already uses.
// shell()'s footer used to hardcode a mailto-only link: Apple Mail/Gmail's
// built-in Unsubscribe button reads List-Unsubscribe and turns a mailto into
// a composed email that nothing processes, so clicking it did nothing.
// Fixed 2026-09-22 after a real report on the giveaway welcome mail.
...
function winnerEmailPlain({ ga, winnerName, claimDays = CLAIM_DAYS }) {
  ...
  const lines = [
    `${winnerName} you won!! 🎉`,
    `I put every single entry in and pulled one at random and it was yours. You're getting a gift card for ${ga.title} by ${ga.author}.`,
    // Asks for NOTHING but a reply. The first giveaway's winner did not want to
    // hand over her full name and address, which is why the prize is a gift card
    // now: it goes to the address that entered, so there is nothing to collect.
    `Just hit reply and I'll send it straight to this email. No address, no forms, that's the whole thing.`,
    `One thing I feel a bit weird saying but I have to: if I don't hear back from you by ${claimBy} I'll have to draw someone else. Genuinely not rushing you, I just don't want it sitting in limbo while someone else could have it.`,
...
function winnerEmail({ ga, winnerName, claimDays = CLAIM_DAYS, email }) {
  const claimBy = claimByDate(claimDays);
  const body = [
    `${winnerName} YOU WON!! 🎉`,
    `Okay so I put every single entry in and pulled one at random and it was YOURS. You're getting a gift card for <b>${esc(ga.title)}</b> 🎉`,
    `So the boring bit: just hit reply and I'll send it straight to this email. No address, no forms, that's it.`,
    `One thing I feel a bit weird saying but I have to: if I don't hear back from you by <b>${claimBy}</b> I'll have to draw someone else 😬 Genuinely NOT rushing you, I just don't want it sitting in limbo while someone else could have it.`,
    `Anyway CONGRATS 🎉 and thank you for playing, it honestly means a lot that people entered at all 💜`,
    `Olga`,
  ];
  return {
...
function listEmail({ ga, winnerName, entries, amzTag, email }) {
...
function shell({ ga, eyebrow, body, cta, secondary, cover, coverTitle, coverAfter, cardButton, buy, disclosure, unsubUrl }) {
      ...
      <table role="presentation" width="100%" align="center" cellpadding="0" cellspacing="0" border="0" style="max-width:480px;margin-left:auto;margin-right:auto;background:#ffffff;border:1px solid #ead4e2;border-radius:14px;padding:32px 28px;">
        <tr><td>
          ...
          <p style="margin:0;font-size:11px;color:#a587a9;line-height:1.6;">
            ...
            <a href="${esc(unsubUrl || 'mailto:booky@90books.com?subject=unsubscribe')}" style="color:#a587a9;text-decoration:underline;">Unsubscribe</a>
          </p>
        </td></tr>
      </table>
    </td></tr>
