// lib/unsubscribe-handler.js — the destination of the footer link in Booky email.
//
...
//
// ⚠️ This lives in lib/ and is dispatched from api/booky-subscribe.js
// (`?unsub=1`) rather than being its own api/ route. The Vercel Hobby plan caps
// a deployment at 12 Serverless Functions and api/ was already at exactly 12 —
// adding a 13th file failed the whole build, which meant NOTHING deployed, not
// just the new route. Anything in lib/ is bundled into its caller and is free.
// Before adding a file to api/, count first.
//
...
const { verify, normalize } = require('./unsubscribe');
  ...
  try {
    const r = await fetch(
      `https://api.resend.com/audiences/${RESEND_AUDIENCE_ID}/contacts/${encodeURIComponent(email)}`,
      {
        ...
        body: JSON.stringify({
          unsubscribed: true,
          // Dashboard-visible only, see api/booky-subscribe.js. A REAL
          // opt-out, as opposed to a signup that never confirmed — the two
          // used to be indistinguishable in the Audience table.
          properties: [{ key: 'status', value: 'unsubscribed' }],
        }),
      }
    );
