    ...
    const r = await fetch(
      `https://api.resend.com/audiences/${RESEND_AUDIENCE_ID}/contacts/${encodeURIComponent(email)}`,
      {
        ...
        body: JSON.stringify({
          ...
          // Resend expects `properties` as a record, not an array of
          // {key,value} pairs — the array shape 422s with "expected record,
          // received array" on every PATCH, which meant every confirm click
          // failed and no contact ever moved past pending. Found 2026-09-22
          // via a debug probe on this exact endpoint.
