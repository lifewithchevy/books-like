  ...
  sendPage(
    ...
    g
      ? `You're entered to win ${esc(g.title || 'the book')}, and I'll announce the winner on ${esc(g.announce || 'the announce date')}. Your daily Booky reminder starts tomorrow.`
        ...
        // Deliberately NOT shrunk or greyed: terms set in smaller type than the
        // text around them read as something being hidden.
        + `<br><br>Open worldwide. The winner gets a gift card for the book, so it works wherever you are.`
      : 'Your daily Booky reminder starts tomorrow. Your streak is safe from here.',
    false
  );
};

// `cta` is false on the success page only. Everywhere else this page appears
// something went wrong and the fix is to go play and sign up again, so the
// button is the next step. On success it is not: the only route to a giveaway
// entry is the win screen, so anyone reading that page finished today's word
// minutes ago and the button offers them the puzzle they just solved.
function sendPage(res, status, title, body, cta = true) {
...
<body style="margin:0;padding:0;background:#fff8fb;font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',Roboto,Inter,sans-serif;color:#2a0a26;">
  <div style="max-width:420px;margin:0 auto;padding:64px 20px;text-align:center;">
    ...
    <div style="background:#ffffff;border:1px solid #ead4e2;border-radius:14px;padding:32px 24px;">
      ...
      <p style="margin:0;font-size:14px;line-height:1.6;color:#6a4a6c;">${body}</p>
      ${cta ? `<p style="margin:24px 0 0;"><a href="https://90books.com/booky?utm_source=confirm&utm_medium=email&utm_campaign=double_optin" style="display:inline-block;background:linear-gradient(135deg,#c8398f,#9a2670);color:#ffffff;text-decoration:none;font-weight:600;padding:13px 28px;border-radius:10px;font-size:15px;">Play today's Booky</a></p>` : ''}
    </div>
  </div>
</body>
</html>`);
