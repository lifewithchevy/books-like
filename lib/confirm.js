...
// `tag` is the giveaway entry tag, carried so the landing page can say what
// they actually confirmed instead of only mentioning the daily reminder. It is
// deliberately NOT part of the signature: it selects copy and nothing else, and
// the handler only honours a tag that matches a giveaway currently live in
// words.json, so the worst a tampered value can do is show the generic page.
function confirmUrl(email, tag) {
  const e = normalize(email);
  const base = `${BASE}/api/booky-subscribe?confirm=1&e=${encodeURIComponent(e)}&t=${sign(e)}`;
  return tag ? `${base}&g=${encodeURIComponent(String(tag).slice(0, 60))}` : base;
}
