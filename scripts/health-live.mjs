...
const DICT = 'https://booky-deploy.vercel.app/booky/dictionary.json';
...
async function main() {
  ...
  if (!dr.ok) { fail(`dictionary returned ${dr.status}`); return done(); }
  const dj = await dr.json();
  const dict = new Set((Array.isArray(dj) ? dj : dj.words || []).map((x) => String(x).toUpperCase()));
